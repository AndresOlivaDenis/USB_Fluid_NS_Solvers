...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [us,vs,aPus,aPvs] = MtumPseudoEq_STFg_asFunction(Nx,Ny,...
...
us=u0; vs=v0; % Para inicio del Proceso iterativo de solucion
%%% Condiciones de borde %%%
% Normal Velocities
...
for i=2:Nx
    for j=2:Ny+1
        ...
        De=miu0(i+1,j)*(b(i+1,j)*(dy_un(i,j)+dy_su(i,j)))/dx_uE(i,j);
        %aEuCero(i,j)=-0.5*Fe+De;
        
        Fw=0.5*(b(i,j)*(dy_un(i,j)+dy_su(i,j)))*...
            ro0(i,j)*(u0(i-1,j)+u0(i,j));
        Dw=miu0(i,j)*(b(i,j)*(dy_un(i,j)+dy_su(i,j)))/dx_Wu(i,j);
...
for i=2:Nx+1
    for j=2:Ny
        ...
        Dn=miu0(i,j+1)*(b(i,j+1)*(dx_wv(i,j)+dx_ve(i,j)))/dy_vN(i,j);
        %aNvCero(i,j)=-0.5*Fn+Dn;
        
        Fs=0.5*(b(i,j)*(dx_wv(i,j)+dx_ve(i,j)))*...
            ro0(i,j)*(v0(i,j-1)+v0(i,j));
        Ds=miu0(i,j)*(b(i,j)*(dx_wv(i,j)+dx_ve(i,j)))/dy_Sv(i,j);
...
while (Error>=Tol)&&(nitera<=nmax) % -> Esto es si se usa metodo iterativo!
    ...
    for i=2:Nx
        for j=2:Ny+1
            Fe=0.5*(b(i+1,j)*(dy_un(i,j)+dy_su(i,j)))*...
                ro(i+1,j)*(us(i,j)+us(i+1,j));
            De=miu(i+1,j)*(b(i+1,j)*(dy_un(i,j)+dy_su(i,j)))/dx_uE(i,j);
            aEu(i,j)=-fc*0.5*Fe+fd*De;
            
            Fw=0.5*(b(i,j)*(dy_un(i,j)+dy_su(i,j)))*...
                ro(i,j)*(us(i-1,j)+us(i,j));
            Dw=miu(i,j)*(b(i,j)*(dy_un(i,j)+dy_su(i,j)))/dx_Wu(i,j);
    ...
    for i=2:Nx+1
        for j=2:Ny
            ...
            Fn=0.5*(b(i,j+1)*(dx_wv(i,j)+dx_ve(i,j)))*...
                ro(i,j+1)*(vs(i,j+1)+vs(i,j));
            Dn=miu(i,j+1)*(b(i,j+1)*(dx_wv(i,j)+dx_ve(i,j)))/dy_vN(i,j);
            aNv(i,j)=-fc*0.5*Fn+fd*Dn;
            
            Fs=0.5*(b(i,j)*(dx_wv(i,j)+dx_ve(i,j)))*...
                ro(i,j)*(vs(i,j-1)+vs(i,j));
            Ds=miu(i,j)*(b(i,j)*(dx_wv(i,j)+dx_ve(i,j)))/dy_Sv(i,j);
