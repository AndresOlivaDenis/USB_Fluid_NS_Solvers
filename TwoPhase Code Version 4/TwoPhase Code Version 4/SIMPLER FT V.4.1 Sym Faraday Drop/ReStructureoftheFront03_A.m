...
function [xFront,yFront,xmapFront,ymapFront]...
    =ReStructureoftheFront03_A(NFront,xFront,yFront,...
...
for il=2:NFront+1
    DistToT=DistToT+...
        min([((xFront(il)-xFront(il-1))^2+(yFront(il)-yFront(il-1))^2)^0.5,...
        ((xFront(il)-xFront(il-1)-Lx)^2+(yFront(il)-yFront(il-1))^2)^0.5,...
        ((xFront(il)-xFront(il-1)+Lx)^2+(yFront(il)-yFront(il-1))^2)^0.5]);
end
...
for il=1:NFront
    ...
    while(Asigil==0)
        if (DeltaiFront>=DistFrontLast0)&&(DeltaiFront<DistFrontLast1)
            ...
            xlastm1=[xmapFrontLast(il0-1),xmapFrontLast(il0-1)+1,...
                xmapFrontLast(il0-1)];
            xlastz=[xmapFrontLast(il0),xmapFrontLast(il0),...
                xmapFrontLast(il0)+1];
            xmapFront(il)=(1-dist0)*xlastm1(idF)+dist0*xlastz(idF);
            xmapFront(il)=xmapFront(il)-floor(xmapFront(il)/1)*1;
            
            ymapFront(il)=(1-dist0)*ymapFrontLast(il0-1)+...
                dist0*ymapFrontLast(il0);
            ...
            [distanceNFront,idF]=min([((xFrontLast(il0)-xFrontLast(il0-1))^2+...
                (yFrontLast(il0)-yFrontLast(il0-1))^2)^0.5,...
                ((xFrontLast(il0)-xFrontLast(il0-1)-Lx)^2+...
                (yFrontLast(il0)-yFrontLast(il0-1))^2)^0.5,...
                ((xFrontLast(il0)-xFrontLast(il0-1)+Lx)^2+...
                (yFrontLast(il0)-yFrontLast(il0-1))^2)^0.5]);
            DistFrontLast0=DistFrontLast1;
            DistFrontLast1=DistFrontLast1+distanceNFront;
        end
    end
