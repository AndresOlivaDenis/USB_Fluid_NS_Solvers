...
% 2D Finite Volumes
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [p,niteraPE,ErrorPE] = PressureEq_4_asFunction(Nx,Ny,...
    dx_Pe,dy_Pn,dx_wP,dy_sP,dy_un,dy_su,dx_ve,dx_wv,...
    ro,b,us,vs,aPus,aPvs,p,dt,ro0,nmaxP,TolP)

% The Pressure Correction Equation
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ErrorPE=1; niteraPE=0;  
...
for i=2:Nx+1
    for j=2:Ny+1
        aEpc(i,j)=0.5*(ro(i,j)*b(i,j)+ro(i+1,j)*b(i+1,j))*(dy_Pn(i,j)+dy_sP(i,j))*...
            0.5*(b(i,j)+b(i+1,j))*(dy_un(i,j)+dy_su(i,j))/aPus(i,j);
        aWpc(i,j)=0.5*(ro(i-1,j)*b(i-1,j)+ro(i,j)*b(i,j))*(dy_Pn(i,j)+dy_sP(i,j))*...
            0.5*(b(i-1,j)+b(i,j))*(dy_un(i-1,j)+dy_su(i-1,j))/aPus(i-1,j);
        aNpc(i,j)=0.5*(ro(i,j)*b(i,j)+ro(i,j+1)*b(i,j+1))*(dx_Pe(i,j)+dx_wP(i,j))*...
            0.5*(b(i,j)+b(i,j+1))*(dx_ve(i,j)+dx_wv(i,j))/aPvs(i,j);
        aSpc(i,j)=0.5*(ro(i,j-1)*b(i,j-1)+ro(i,j)*b(i,j))*(dx_Pe(i,j)+dx_wP(i,j))*...
            0.5*(b(i,j-1)+b(i,j))*(dx_ve(i,j-1)+dx_wv(i,j-1))/aPvs(i,j-1);
...
while (ErrorPE>=TolP)&&(niteraPE<=nmaxP) % -> Esto es si se usa metodo iterativo!
   ...
   % x linea horizontal
    for i=2:Nx+1
        ...
        dy(jM)=bpc(i,j)+...
            aEpc(i,j)*p(i+1,j)+aWpc(i,j)*p(i-1,j)+aNpc(i,j)*p(i,j-1);
        ...
        j=Ny+1; jM=j-1;
        dy(jM)=bpc(i,j)+...
            aEpc(i,j)*p(i+1,j)+aWpc(i,j)*p(i-1,j)+aSpc(i,j)*p(i,j-1); 
        My(jM,jM)=aPpc(i,j); My(jM,jM-1)=-aSpc(i,j);
        
        % Solucion
        p(i,2:Ny+1)=TDMA(My,dy);
    end
    % x Linea Vertical
    for j=2:Ny+1
        ...
        dx(iM)=bpc(i,j)+...
            aNpc(i,j)*p(i,j+1)+aSpc(i,j)*p(i,j-1)+aWpc(i,j)*p(i-1,j);
        ...
        i=Nx+1; iM=i-1;
        dx(iM)=bpc(i,j)+...
            aNpc(i,j)*p(i,j+1)+aSpc(i,j)*p(i,j-1)+aEpc(i,j)*p(i+1,j); 
        Mx(iM,iM)=aPpc(i,j); Mx(iM,iM-1)=-aWpc(i,j);
        
        % Solucion
        p(2:Nx+1,j)=TDMA(Mx,dx);
    end
