...
function [pc,niteraPC,ErrorPC] = PressureCorrecRo2_3_Perio(Nx,Ny,...
...
% Nodos Periodicidad en x1 -----------------------------------------------
i=2; j=2; aWpc(i,j)=0.5*(aEpc(i,j)+aWpc(Nx+1,j));
aPpc(i,j)=aWpc(i,j)+aEpc(i,j)+aNpc(i,j); % Esquina 1

i=2; % Nodos del tope izquierdo
for j=3:Ny, aWpc(i,j)=0.5*(aEpc(i,j)+aWpc(Nx+1,j));
    aPpc(i,j)=aWpc(i,j)+aEpc(i,j)+aNpc(i,j)+aSpc(i,j);end

i=2; j=Ny+1;aWpc(i,j)=0.5*(aEpc(i,j)+aWpc(Nx+1,j));
aPpc(i,j)=aWpc(i,j)+aEpc(i,j)+aSpc(i,j); % Esquina  3
% ------------------------------------------------------------------------
...
% Nodos Periodicidad en x2 -----------------------------------------------
i=Nx+1; j=2; aEpc(i,j)=0.5*(aEpc(2,j)+aWpc(i,j));
aPpc(i,j)=aEpc(i,j)+aWpc(i,j)+aNpc(i,j); % Esquina 2

i=Nx+1; % Nodos del tope derecho
for j=3:Ny, aEpc(i,j)=0.5*(aEpc(2,j)+aWpc(i,j));
    aPpc(i,j)=aEpc(i,j)+ aWpc(i,j)+aNpc(i,j)+aSpc(i,j); end

i=Nx+1; j=Ny+1; aEpc(i,j)=0.5*(aEpc(2,j)+aWpc(i,j));
aPpc(i,j)=aEpc(i,j)+aWpc(i,j)+aSpc(i,j); % Esquina  4
% ------------------------------------------------------------------------
...
while (ErrorPC>=TolP)&&(niteraPC<=nmaxP) % -> Esto es si se usa metodo iterativo!
    ...
    % Condicion de borde de periodicidad ---------------------------------
    pc(1,:)=pc(Nx+1,:); pc(Nx+2,:)=pc(2,:);
    % --------------------------------------------------------------------
    for i=2:Nx+1
        for j=2:Ny+1
...
%     ErrorPC=min(ErrorPC1,ErrorPC2*10);
