...
function [MarkF,MarkFdx,MarkFdy]...
    =ConstructMarkerFunction03_C(MarkF0,MF01,MF02,Nx,Ny,...
...
for il=2:NFront+1
    ...
    MarkFdx(ixF,jyF-1)=MarkFdx(ixF,jyF-1)+nfx/(dxu*dyu)*FIx(rAx1)*FIy2(rAy1);
    MarkFdx(ixF+1,jyF-1)=MarkFdx(ixF+1,jyF-1)+nfx/(dxu*dyu)*FIx(rAx2)*FIy2(rAy2);
    MarkFdx(ixF-1,jyF)=MarkFdx(ixF-1,jyF)+nfx/(dxu*dyu)*FIx2(rAx3)*FIy(rAy3);
    MarkFdx(ixF+2,jyF)=MarkFdx(ixF+2,jyF)+nfx/(dxu*dyu)*FIx2(rAx4)*FIy(rAy4);
    MarkFdx(ixF-1,jyF+1)=MarkFdx(ixF-1,jyF+1)+nfx/(dxu*dyu)*FIx2(rAx5)*FIy(rAy5);
    MarkFdx(ixF+2,jyF+1)=MarkFdx(ixF+2,jyF+1)+nfx/(dxu*dyu)*FIx2(rAx6)*FIy(rAy6);
    MarkFdx(ixF,jyF+2)=MarkFdx(ixF,jyF+2)+nfx/(dxu*dyu)*FIx(rAx7)*FIy2(rAy7);
    MarkFdx(ixF+1,jyF+2)=MarkFdx(ixF+1,jyF+2)+nfx/(dxu*dyu)*FIx(rAx8)*FIy2(rAy8);
    MarkFdx(ixF-1,jyF-1)=MarkFdx(ixF-1,jyF-1)+nfx/(dxu*dyu)*FIx2(rAx9)*FIy2(rAy9);
    MarkFdx(ixF+2,jyF-1)=MarkFdx(ixF+2,jyF-1)+nfx/(dxu*dyu)*FIx2(rAx10)*FIy2(rAy10);
    MarkFdx(ixF-1,jyF+2)=MarkFdx(ixF-1,jyF+2)+nfx/(dxu*dyu)*FIx2(rAx11)*FIy2(rAy11);
    MarkFdx(ixF+2,jyF+2)=MarkFdx(ixF+2,jyF+2)+nfx/(dxu*dyu)*FIx2(rAx12)*FIy2(rAy12);
    % -------------------------------------------------------------------
    
    % Flags x -----------------------------------------------------------
    Flagx(ixF,jyF)=1; Flagx(ixF+1,jyF)=1; Flagx(ixF+2,jyF)=1; 
    Flagx(ixF-1,jyF)=1; Flagx(ixF+3,jyF)=1;
    Flagx(ixF,jyF+1)=1; Flagx(ixF+1,jyF+1)=1; Flagx(ixF+2,jyF+1)=1; 
    Flagx(ixF-1,jyF+1)=1; Flagx(ixF+3,jyF+1)=1;
    Flagx(ixF,jyF+2)=1; Flagx(ixF+1,jyF+2)=1; Flagx(ixF+2,jyF+2)=1; 
    Flagx(ixF-1,jyF+2)=1; Flagx(ixF+3,jyF+2)=1;
    Flagx(ixF,jyF-1)=1; Flagx(ixF+1,jyF-1)=1; Flagx(ixF+2,jyF-1)=1; 
    Flagx(ixF-1,jyF-1)=1; Flagx(ixF+3,jyF-1)=1;
    % -------------------------------------------------------------------
    ...
    MarkFdy(ixF,jyF-1)=MarkFdy(ixF,jyF-1)+nfy/(dxv*dyv)*FIx(rAx1)*FIy2(rAy1);
    MarkFdy(ixF+1,jyF-1)=MarkFdy(ixF+1,jyF-1)+nfy/(dxv*dyv)*FIx(rAx2)*FIy2(rAy2);
    MarkFdy(ixF-1,jyF)=MarkFdy(ixF-1,jyF)+nfy/(dxv*dyv)*FIx2(rAx3)*FIy(rAy3);
    MarkFdy(ixF+2,jyF)=MarkFdy(ixF+2,jyF)+nfy/(dxv*dyv)*FIx2(rAx4)*FIy(rAy4);
    MarkFdy(ixF-1,jyF+1)=MarkFdy(ixF-1,jyF+1)+nfy/(dxv*dyv)*FIx2(rAx5)*FIy(rAy5);
    MarkFdy(ixF+2,jyF+1)=MarkFdy(ixF+2,jyF+1)+nfy/(dxv*dyv)*FIx2(rAx6)*FIy(rAy6);
    MarkFdy(ixF,jyF+2)=MarkFdy(ixF,jyF+2)+nfy/(dxv*dyv)*FIx(rAx7)*FIy2(rAy7);
    MarkFdy(ixF+1,jyF+2)=MarkFdy(ixF+1,jyF+2)+nfy/(dxv*dyv)*FIx(rAx8)*FIy2(rAy8);
    MarkFdy(ixF-1,jyF-1)=MarkFdy(ixF-1,jyF-1)+nfy/(dxv*dyv)*FIx2(rAx9)*FIy2(rAy9);
    MarkFdy(ixF+2,jyF-1)=MarkFdy(ixF+2,jyF-1)+nfy/(dxv*dyv)*FIx2(rAx10)*FIy2(rAy10);
    MarkFdy(ixF-1,jyF+2)=MarkFdy(ixF-1,jyF+2)+nfy/(dxv*dyv)*FIx2(rAx11)*FIy2(rAy11);
    MarkFdy(ixF+2,jyF+2)=MarkFdy(ixF+2,jyF+2)+nfy/(dxv*dyv)*FIx2(rAx12)*FIy2(rAy12);
    % -------------------------------------------------------------------
    ...
    Flagy(ixF+2,jyF)=1; Flagy(ixF+2,jyF+1)=1; Flagy(ixF+2,jyF+2)=1;
    Flagy(ixF+2,jyF-1)=1; Flagy(ixF+2,jyF+3)=1;
    Flagy(ixF-1,jyF)=1; Flagy(ixF-1,jyF+1)=1; Flagy(ixF-1,jyF+2)=1;
    Flagy(ixF-1,jyF-1)=1; Flagy(ixF-1,jyF+3)=1;
    % -------------------------------------------------------------------
end
