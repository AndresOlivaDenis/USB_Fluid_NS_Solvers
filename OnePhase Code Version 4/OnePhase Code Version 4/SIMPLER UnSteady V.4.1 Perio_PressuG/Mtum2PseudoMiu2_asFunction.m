...
function [us,vs,aPus,aPvs] = Mtum2PseudoMiu2_asFunction(Nx,Ny,...
...
us(:,1) = u0(:,Ny+1);         us(:,Ny+2) = u0(:,2);
% -----------------------------------------------------------------------
% Normal V Velocity -----------------------------------------------------
vs(2:Nx+1,1) = v0(2:Nx+1,Ny) + (dy_sP(2:Nx+1,Ny+1)+dy_Pn(2:Nx+1,Ny+1))...
    ./(dx_Pe(2:Nx+1,Ny+1)+dx_wP(2:Nx+1,Ny+1)).*(u0(1:Nx,Ny+1)-u0(2:Nx+1,Ny+1));
vs(2:Nx+1,Ny+1) = v0(2:Nx+1,2) + (dy_sP(2:Nx+1,2)+dy_Pn(2:Nx+1,2))...
    ./(dx_Pe(2:Nx+1,2)+dx_wP(2:Nx+1,2)).*(-u0(1:Nx,2)+u0(2:Nx+1,2));
% -----------------------------------------------------------------------
% "Smoothing" periodicity -.-.-.-.-.-.-.-.-.-.-.-.-.-.-
vs(2:Nx+1,1)=0.5*(vs(2:Nx+1,1)+vs(2:Nx+1,Ny+1)); vs(2:Nx+1,Ny+1)=vs(2:Nx+1,1);
% -----------------------------------------------------------------------
...
vs(1,:)=(2*vCBwest-v0(2,:))*(1-CvCB(1))+(v0(2,:)-dx_vE(1,:)*vCBwest)*CvCB(1);
vs(Nx+2,:)=(2*vCBeast-v0(Nx+1,:))*(1-CvCB(3))+(v0(Nx+1,:)+dx_Wv(Nx+2,:)*vCBeast)*CvCB(3);
% =======================================================================
