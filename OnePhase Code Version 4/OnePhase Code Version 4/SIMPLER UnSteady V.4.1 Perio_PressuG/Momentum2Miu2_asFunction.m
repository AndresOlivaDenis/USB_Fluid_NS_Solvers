...
% Time -> General CN                          Space -> Centered Diference 
% ========================================================================

function [ut,vt,aPu,aPv] = Momentum2Miu2_asFunction(Nx,Ny,...
...
Error=100; nitera=0; nmax=20; % omeu=1.5; omev=1.5;
...
[ut,vt] = Velocities_BC_Perioy(uCBeast, uCBwest,...
...
while (Error>=Tol)&&(nitera<=nmax) % -> Esto es si se usa metodo iterativo!
    ...
    for i=2:Nx+1
        for j=2:Ny
            ...
            Fn=0.5*(b(i,j+1)*(dx_wv(i,j)+dx_ve(i,j)))*...
                0.5*(ro(i,j)+ro(i,j+1))*(vt(i,j+1)+vt(i,j));
...
%     
    Erroru=(sum((ut-ulast).^2)/sum(ut.^2))^0.5;
    Errorv=(sum((vt-vlast).^2)/sum(vt.^2))^0.5;
    Error=max(Erroru,Errorv)*100;
    % ========================================================================
    % Notas:
