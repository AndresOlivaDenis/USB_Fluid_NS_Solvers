...
[u0,v0] = Velocities_BC_Perioy(uCBeast, uCBwest,...
...
umiddle = zeros(Nt+1,1); diffUdt = zeros(Nt+1,1); dt0=dt;
% =======================================================================
...
for it=floor(((iinte-1)*Nt/NInter+1)):floor(((iinte)*Nt/NInter))
    ...
    while (Error>=Tol*2)&&(nitera<=nmax)
        ...
        [us,vs,aPus,aPvs] = Mtum2PseudoMiu2_asFunction(Nx,Ny,...
            ...
            u0,v0,ro0,miu0,dt,...
            uCBwest,uCBeast,vCBwest,vCBeast, dy_Pn, dy_sP, dx_wP, dx_Pe,...
            CuCB,CvCB,fcPV,fdPV,fSuPV,fSvPV,...
            Tol,us,vs);
        % --------------------------------------------------------------
        
        % The Pressure Equation ----------------------------------------
        [p,niteraPE,ErrorPE] = PressureRo2_3PG_asFunction(Nx,Ny,...
...
%         [pc,niteraPC,ErrorPC] = PressureCorrecRo2_3PG_asFunction(Nx,Ny,...
%             dx_Pe,dy_Pn,dx_wP,dy_sP,dy_un,dy_su,dx_ve,dx_wv,...
...
%       % -.-.-.-.--.-.-.-.--.-.-.-.--.-.-.-.-
        ...
        [u,v] = Velocities_BC_Perioy(uCBeast, uCBwest,...
            vCBwest, vCBeast, CuCB, CvCB,...
            dx_uE, dx_Wu, dx_Wv, dx_vE, dy_Pn, dy_sP,  dx_wP, dx_Pe,...
            Nx,Ny,u,v);
        % --------------------------------------------------------------
    ...
    % ---------------------------------------------------------------
    umiddle(it) = 0.5*(v(floor(Nx*0.5)+1,3) + v(floor(Nx*0.5)+2,3)); 
    diffUdt(it) = max(max(abs(v-v0)))/dt*1e6;
%     dt = dt0*(1 - 0.9*(it/Nt));
    % ---------------------------------------------------------------
...
% =======================================================================
figure; plot(1:it,umiddle(1:it));

yteo = -Lx*0.5:0.01:Lx*0.5; uteo = umax*(1 - yteo.^2/(0.5*Lx)^2);
yteo = yteo + 0.5*Lx;
figure; plot(xv(:,3),-v(:,3),'o',...
    yteo,uteo);
% =======================================================================
